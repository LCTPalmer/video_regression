...
class MultiChannelModel():
    ...
    model - class of sklearn model:
        e.g. sklearn.svm.SVR (which is default)
    ...
    def __init__(self, num_channels, model=SVR,  model_param_dict={'C': 1}, kernel_param_tuple=None):
        
        self.num_channels = num_channels
        self.model_param_dict = model_param_dict
        self.model = model(kernel='precomputed', **self.model_param_dict) # TODO checks on param dict etc
...
def multichannel_KFoldCV(model, X, y, n_folds=3, verbose=False):
    ...
    assert isinstance(model, MultiChannelModel), 'model must be instance of MultiChannelSVR class'
